"""
Transfer-Learning Ensemble with Uncertainty Quantification for Band Gap Prediction
===================================================================================
Nested cross-validation (5-outer × 5-inner) using a deep ensemble of fine-tuned
transfer-learning models and isotonic recalibration via the uncertainty-toolbox
library.
...
Requirements
------------
    pip install tensorflow uncertainty-toolbox scikit-learn matplotlib numpy

Data
----
    ECFPs_Scan_466.npy          – ECFP fingerprint matrix (N × D)
    Bandgaps_Scan_466.npy       – Band-gap labels in eV  (N,)
    infinity_feat_index_Scan.npy   – Indices of infinite-valued features to drop
    MP_16770_Scan.h5               – Pre-trained Keras base model

Usage
-----
    python ensemble_transfer_bandgap_uq.py
"""
...
BASE_MODEL_PATH = Path("MP_16770_Scan.h5")
...
TRAIN_CFG = dict(
    ...
    batch_size=32,
    patience=200,
    n_trainable_layers=5,   # number of base-model layers to unfreeze
)

# Best learning rates found during inner-CV tuning, per outer fold × inner fold
OUTER_INNER_BEST_HPS: List[List[dict]] = [
    [  # Outer fold 1
        {"learning_rate": 2.55e-05},
        {"learning_rate": 9.997374120547577e-05},
        {"learning_rate": 8.678176274338925e-05},
        {"learning_rate": 3.755877233525119e-05},
        {"learning_rate": 2.481027588351326e-05},
    ],
    [  # Outer fold 2
        {"learning_rate": 1e-05},
        {"learning_rate": 0.0001},
        {"learning_rate": 4.9944545249710225e-05},
        {"learning_rate": 1.9305360776647966e-05},
        {"learning_rate": 3.0420614174917535e-05},
    ],
    [  # Outer fold 3
        {"learning_rate": 9.947147929835256e-05},
        {"learning_rate": 3.204934447969946e-05},
        {"learning_rate": 9.73714329555794e-05},
        {"learning_rate": 0.0001},
        {"learning_rate": 6.870252393108731e-05},
    ],
    [  # Outer fold 4
        {"learning_rate": 0.0001},
        {"learning_rate": 6.849129307330532e-05},
        {"learning_rate": 9.237908330090376e-05},
        {"learning_rate": 0.0001},
        {"learning_rate": 8.061923671260944e-05},
    ],
    [  # Outer fold 5
        {"learning_rate": 1e-05},
        {"learning_rate": 1.5706526969486712e-05},
        {"learning_rate": 8.337316935315421e-05},
        {"learning_rate": 5.500425487010883e-05},
        {"learning_rate": 3.108150070712184e-05},
    ],
]
...
def build_model(hp: dict, input_shape: tuple, seed: int = SEED) -> keras.Model:
    """Load the pre-trained base model and wrap it in a fine-tunable Keras Model.

    The first ``TRAIN_CFG['n_trainable_layers']`` layers of the base model are
    set trainable; all others remain frozen.

    Parameters
    ----------
    hp:
        Dictionary with key ``learning_rate``.
    input_shape:
        Shape of a single input sample, e.g. ``(2048,)`` for ECFP-2048.
    seed:
        Random seed for weight initialisation of the Adam optimiser state.

    Returns
    -------
    keras.Model
        Compiled Keras model ready for fine-tuning.
    """
    ...
    for i in range(TRAIN_CFG["n_trainable_layers"]):
        base_model.layers[i].trainable = True

    model_input = Input(shape=input_shape, name="input_layer", dtype="float32")
    output = base_model(model_input)
...
def run_nested_cv(
    ...
    """Run nested k-fold cross-validation with transfer-learning ensemble training
    and isotonic recalibration.
...
def plot_calibration_comparison(
    ...
    for label, y_std, color in [
        ...
        ("Calibrated",   y_std_recal, "darkblue"),
    ]:
        exp_p, obs_p = uct.metrics_calibration.get_proportion_lists_vectorized(
            y_pred, y_std, y_true, num_bins=num_bins, prop_type="interval"
        )
        ax.plot(exp_p, obs_p, lw=2.5, label=label, color=color)
        ax.fill_between(exp_p, exp_p, obs_p, alpha=0.2, color=color)

    ma_uncal = uct.metrics_calibration.miscalibration_area(
        y_pred, y_std_uncal, y_true, num_bins=num_bins, vectorized=True, prop_type="interval"
    )
    ma_recal = uct.metrics_calibration.miscalibration_area(
        y_pred, y_std_recal, y_true, num_bins=num_bins, vectorized=True, prop_type="interval"
    )
    reduced = ma_uncal - ma_recal
...
def plot_sharpness_comparison(
    ...
    ax.hist(y_std_uncal, bins=bin_edges, color="darkred",  edgecolor="k",
            density=True, alpha=0.6, label="Uncalibrated")
    ax.hist(y_std_recal, bins=bin_edges, color="darkblue", edgecolor="k",
            density=True, alpha=0.6, label="Calibrated")

    rms_uncal = float(np.sqrt(np.mean(y_std_uncal ** 2)))
    rms_recal = float(np.sqrt(np.mean(y_std_recal ** 2)))
    ax.axvline(rms_uncal, color="darkred",  ls="--", lw=2.5,
               label=f"Sharpness (uncal) = {rms_uncal:.2f} eV")
    ax.axvline(rms_recal, color="darkblue", ls="--", lw=2.5,
               label=f"Sharpness (cal)   = {rms_recal:.2f} eV")
...
def plot_parity_overlay(
    ...
    for label, y_std, color in [
        ...
        ax.scatter(y_true, y_pred, color=color, alpha=0.5, s=25,
                   label=label, zorder=3)
        ax.errorbar(y_true, y_pred, yerr=y_std, fmt="none",
                    ecolor=color, elinewidth=2.0, alpha=0.4, capsize=3, zorder=2)
